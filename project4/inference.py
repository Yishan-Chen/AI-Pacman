...
class ExactInference(InferenceModule):
        ...
        if noisyDistance is None:
            ...
            for ghostPosition in self.legalPositions:
                ...
                if emissionModel[trueDistance] > 0:
                    allPossible[ghostPosition] = self.beliefs[ghostPosition] * emissionModel[trueDistance]
    ...
    def elapseTime(self, gameState):
        ...
        """
        possibleValue = util.Counter()
        for oldPos in self.legalPositions:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            for newPos, prob in newPosDist.items():
                possibleValue[newPos] += self.beliefs[oldPos] * prob

        possibleValue.normalize()
        self.beliefs = possibleValue
        "*** YOUR CODE HERE ***"
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            for ghostPosition in self.particles:
                trueDistance = util.manhattanDistance(ghostPosition, pacmanPosition)
                if emissionModel[trueDistance] > 0:
                    weightedParticle[ghostPosition] += emissionModel[trueDistance]
            weightedParticle.normalize()
            if weightedParticle.totalCount() == 0:
                self.initializeUniformly(gameState)
            else:
                self.particles = [util.sample(weightedParticle) for _ in range(self.numParticles)]
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.particles:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            possibleParticles.append(util.sample(newPosDist))
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        for _ in range(self.numParticles):
            ...
            self.particles.append(tuple(ghostList))
    ...
    def observeState(self, gameState):
        ...
        for ghostPositions in self.particles:
            ...
            for i in range(self.numGhosts):
                if noisyDistances[i] is not None:
                    trueDistance = util.manhattanDistance(ghostPositions[i], pacmanPosition)
                    weight *= emissionModels[i][trueDistance]
            weightedParticle[ghostPositions] += weight
        weightedParticle.normalize()
        ...
        if weightedParticle.totalCount() == 0:
            ...
            for i in range(self.numParticles):
                particle = util.sample(weightedParticle)
                newParticle = [None] * len(particle)
                for ghostIndex, ghostPosition in enumerate(noisyDistances):
                    if ghostPosition is None:
                        newParticle[ghostIndex] = self.getJailPosition(ghostIndex)
                    else:
                        newParticle[ghostIndex] = particle[ghostIndex]
                newParticles.append(tuple(newParticle))
            self.particles = newParticles
