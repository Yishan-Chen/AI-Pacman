...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = [beliefs for i, beliefs
                                            ...
                                            if livingGhosts[i + 1]]
        mostPossibleGhost = util.Counter()
        for index, ghost in enumerate(livingGhostPositionDistributions):
            currentProb = 0
            for pos, prob in ghost.items():
                if prob > currentProb:
                    mostPossibleGhost[index] = pos
                    currentProb = prob

        currentDict = float('inf')
        bestAction = None
        for index, pos in mostPossibleGhost.items():
            for action in legal:
